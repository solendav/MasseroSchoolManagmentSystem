...
<?php
	...
	include('assets/inc/config.php');
		if(isset($_POST['update_Student']))
		{
			...
			$query="UPDATE  his_students  SET pat_fname=?, pat_lname=?, pat_age=?, pat_dob=?,  pat_phone=?, classArmId=?, pat_addr=?, classId=? WHERE admissionNumber=?";
			$stmt = $mysqli->prepare($query);
			$rc=$stmt->bind_param('sssssssss', $pat_fname, $pat_lname, $pat_age, $pat_dob,  $pat_phone, $classArmId, $pat_addr, $classId, $admissionNumber);
			$stmt->execute();
			/*
            ...
            <div class="content-page">
                <div class="content">
                    ...
                    <div class="container-fluid">
                        ...
                        <?php
                            $admissionNumber=$_GET['admissionNumber'];
                            $ret="SELECT  * FROM his_students WHERE admissionNumber=?";
                            $stmt= $mysqli->prepare($ret) ;
                            $stmt->bind_param('i',$admissionNumber);
                            $stmt->execute() ;//ok
                            $res=$stmt->get_result();
                            //$cnt=1;
                                    ...
                                    <div class="card-body">
                                        ...
                                        <form method="post">
                                            ...
                                            <div class="form-row">
                                                ...
                                                <div class="form-group col-md-4">
                                                    <label for="inputCity" class="col-form-label">ClassId</label>
                                                    <input required="required" type="text" value="<?php echo $row->classId;?>" name="classId" class="form-control" id="inputCity">
                                                </div>
                                                <div class="form-group col-md-4">
                                                    <label for="inputState" class="col-form-label">Section</label>
                                                    <input required="required" type="text" value="<?php echo $row->classArmId;?>" name="classArmId" class="form-control" id="inputCity">
                                               
                                                </div>
                                                
                                            </div>

                                            <button type="submit" name="update_Student" class="ladda-button btn btn-success" data-style="expand-right">Add Student</button>
