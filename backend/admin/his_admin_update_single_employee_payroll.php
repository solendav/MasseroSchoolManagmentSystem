...
<?php
	...
	include('assets/inc/config.php');
		if(isset($_POST['update_payroll']))
		{
			...
			$query="UPDATE   his_payrolls SET pay_staf_name=?, pay_staf_number=?, pay_staf_email=?, pay_emp_salary=?, pay_descr=?, pay_status = ? WHERE pay_number = ?";
			$stmt = $mysqli->prepare($query);
			$rc=$stmt->bind_param('sssssss',  $pay_staf_name, $pay_staf_number, $pay_staf_email, $pay_emp_salary, $pay_descr, $pay_status, $pay_number);
			$stmt->execute();
			/*
                                            ...
                                            <form method="post">
                                                <div class="form-row">
                                                    ...
                                                    <div class="form-group col-md-4">
                                                        ...
                                                        <input type="text" required="required" readonly name="pay_staf_name" value="<?php echo $row->pay_staf_name;?>" class="form-control" id="inputEmail4" placeholder="Patient's Name">
                                                    </div>

                                                    <div class="form-group col-md-4">
                                                        <label for="inputPassword4" class="col-form-label">Employee Email</label>
                                                        <input required="required" type="text" readonly name="pay_staf_email" value="<?php echo $row->pay_staf_email;?>" class="form-control"  id="inputPassword4" placeholder="Patient`s Last Name">
                                                    </div>

                                                    <div class="form-group col-md-4">
                                                        <label for="inputPassword4" class="col-form-label">Employee Number</label>
                                                        <input required="required" type="text" readonly name="pay_staf_number" value="<?php echo $row->pay_staf_number?>" class="form-control"  id="inputPassword4" placeholder="Patient`s Last Name">
                                                    </div>
